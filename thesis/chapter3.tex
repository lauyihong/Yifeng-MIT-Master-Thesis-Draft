...
\chapter{A Spatio-Temporal Extraction Pipeline and Case Study: Massachusetts Covenants Document Analysis}\label{ch:case-study}


[intro and limitation] As mentioned in the last chapter, the current development of Retrieval Augmented Generation architecture allows ai get stronger ability on reasoning and for the ...  well performances, but the gap in the processing the normal, un-structed data to strcutral graphical data which allows RAG. is huge. This chapter presents our first contribution: a document processing pipeline applied to historical deed records from the Massachusetts Covenants Project. We describe the dataset, detail the extraction pipeline, and present findings demonstrating the system's effectiveness for the systematic analysis of discriminatory housing patterns.
...
Background on MassHousing collaboration, project goals, broader dataset context (9300 around total docs, 2300 around docs contain manually review result. the data set we 
